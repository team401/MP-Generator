...
class FalconLinePlot extends JPanel implements ClipboardOwner {
	...
	private double upperXtic,
		...
		upperYtic,
		lowerYtic;
	private boolean xGrid, yGrid;

	private double xMin, xMax, yMin, yMax;

	private int xticCount, yticCount;
	private double xTicStepSize, yTicStepSize;

	public boolean userSetXTic, userSetYTic;

	private String xLabel, yLabel, titleLabel;

	private static int count = 0;

	public JPopupMenu menu = new JPopupMenu("Popup");
	...
	public void setXTic(double lowerBound, double upperBound, double stepSize) {
		userSetXTic = true;
		upperXtic = upperBound;
		lowerXtic = lowerBound;
		xTicStepSize = stepSize;
	}

	public void setYTic(double lowerBound, double upperBound, double stepSize) {
		userSetYTic = true;
		upperYtic = upperBound;
		lowerYtic = lowerBound;
		yTicStepSize = stepSize;
	}
	...
	private void setTitle(Graphics2D g2, String s) {
		...
		for (String str:line) {
			...
			g2.drawString(str, getWidth() / 2 - (width / 2), height);
			height += fm.getHeight();
		}
	}
